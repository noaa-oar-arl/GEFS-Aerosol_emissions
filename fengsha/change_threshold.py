...
if __name__ == "__main__":
    ...
    print('---------------------------------------------------------------------')
    print('---------------------------------------------------------------------')
    f = xr.open_dataset(args.filename, decode_cf=False, mask_and_scale=False, decode_times=False)
    ut = array(args.threshold_velocity.split(','),dtype=float)
    print('     Setting Thresholds...')
    if args.latlon_box is None:
        stype = f.soiltype.data
        thres = f.uthres.copy()
        thres_data = thres.data.squeeze()
        for index,uthres in enumerate(ut):
            con1 = make_condition(x=arange(1,14),soil=index+1,soiltype=f.soiltype)
            #print(uthres)
            thres_data = patch_threshold_array(thres_data,~con1.squeeze(), val=uthres)
        f['uthres'].data = thres_data.reshape(thres.shape)
    else:
        print('Using Lat Lon Box')
        thres = f['uthres'].copy().data
        thresshape = f.uthres.load().shape
        latlonbox = array(args.latlon_box.split(',')).astype(float)
        con2 = make_latlon_con(f,latlonbox=latlonbox)
        for index,uthres in enumerate(ut):
            con1 = make_condition(x=arange(1,14),soil=index+1, soiltype=f.soiltype)
            con = ~con1 & con2
            thres = patch_threshold_array(thres.squeeze(),con, val=uthres)
    print(thres.shape)
